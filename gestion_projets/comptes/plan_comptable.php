...
if(isset($_POST['SauvePlanComptable'])){
	...
	if($SauvePlanComptable=='Enregistrer'){
		$q=$db->prepare("INSERT INTO `gp_plan_comptable`(`compte`, `description`, `Nature`, `Rubrique`) 
       ...
       $saved=$q->rowCount();
		...
		$q=$db->prepare("UPDATE `gp_plan_comptable` SET `compte`=:compte,`description`=:description,
        ...
        $q=$db->prepare("delete from `gp_plan_comptable`  where `id` =:id");
		try{
		$q->execute(array(
			':id'=>$id
        ));
...
<div class="wrapper">
    ...
    <!-------sidebar--design- close----------->
      ...
      <div id="content">
		  ...
		  <!------top-navbar-end-----------> 
		  <?php
		  include("view/plan_comptable.view.php");
		  ?>
		 <!----footer-design------------->
		 <footer class="footer">
