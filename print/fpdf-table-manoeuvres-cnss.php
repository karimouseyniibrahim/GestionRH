 ...
 //$data,$ColWidths,$ColTotal,ETAT_MENSUELE_CNSS.$mois->mois." ".$mois->annee," - DATE D'EDITION : ".date("d/m/Y")
	...
	public function rptDetailData () {
        ...
        foreach($this->data as $pData){
            $this->Row(array($pData->MATRICULEV,$pData->NR_CNSS,$pData->nom,
			number_format($pData->SALABRUT, 0, ',', ' '),number_format($pData->CNSS_RET, 0, ',', ' '),
			number_format(($pData->CNSS_EMP+$pData->ANPE), 0, ',', ' ')));
			$brut+=$pData->SALABRUT;
			$montant+=$pData->CNSS_RET;
			$isc+=($pData->CNSS_EMP+$pData->ANPE);
		...
		$this->Row(array("TOTAL GENERAL",number_format($brut, 0, ',', ' '),
		number_format($montant, 0, ',', ' '),number_format($isc, 0, ',', ' ')));
	...
	function Row($data)
	{
		...
		for($i=0;$i<count($data);$i++)
		{
			...
			$this->Rect($x,$y,$w,$h);
			//Print the text
			$this->MultiCell($w,$this->margintop,$data[$i],0,$a);
	...
	function Header()
{
   ...
   // $this->Cell(80);
    ...
    $this->SetFont("", "B", 15);
		...
		$this->Ln(10);
		$this->Cell(50, 10, utf8_decode($this->edition), 0, 1,'L');
		$this->Ln(10);
    // Line break
    $this->Ln(5);
}
